...
// 			array('pname','require','产品名不能为空'),
// 			array('pmuch','require','价格不能为空'),
// 			array('pamout','require','数量不能为空'),
...
// 	);
	...
	public function depadd(){
		...
		if ($_POST['mouth'] == '') {
			...
			unset($_POST['cycle']);
			unset($_POST['ztime']);
			$_POST['cycle'] = $cycle*30;
			$_POST['ztime'] = $ztime*30;
		}
		$_POST['time']=$_POST['cycle']*24*60*60+time();
		//$_POST['pnumber']=$_POST['mouth'];
		$_POST['nowamout'] = $_POST['pamout'];
	...
	//修改**********************************
	public function changeList($pid){
		$ini=array();
		if(!empty($pid)){
			$ini['pid']=$pid;
		}
